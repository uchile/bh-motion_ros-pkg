...
{
public:
  PassTarget() : playerNumber(0), timeToReachPassPose(0.f) {}
  PassTarget(Vector2<> target, int playerNumber, float timeToReachPassPose) : target(target), playerNumber(playerNumber), timeToReachPassPose(timeToReachPassPose) {}
  Vector2<> target;
  int playerNumber;
  float timeToReachPassPose;
