...
private:
  virtual void serialize(In* in, Out* out)
  {
    ...
    STREAM(timeSinceLastGroundContact);
...
public:
  ...
  BallHypotheses ballHypotheses[numOfPlayers]; /**< The last received ball hypotheses of each team mate. */
  ...
  unsigned int timeSinceLastGroundContact[numOfPlayers]; /**< The time since last ground contact of a team mate. */
  ...
  */
    ...
    wasConnected(false)
  {
    ...
    {
      timeStamps[i] = 0;
      timeSinceLastGroundContact[i] = 0;
    }
  }
    ...
    COMPLEX_DRAWING("representation:TeamMateData",
    {
      for(int i = 1; i < numOfPlayers; ++i)
      {
        if(timeStamps[i])
        {
          ...
          Vector2<> bPos = ballModels[i].estimate.getPositionInFieldCoordinates(robotPoses[i]);
