...
{
  vector<Line> lines;
  vector<LineSegment> singleSegs;
  if(out)
  {
    lines.reserve(this->lines.size());
    for(list<Line>::const_iterator i = this->lines.begin(); i != this->lines.end(); ++i)
      lines.push_back(*i);
    singleSegs.reserve(this->singleSegs.size());
    for(list<LineSegment>::const_iterator i = this->singleSegs.begin(); i != this->singleSegs.end(); ++i)
  ...
  if(in)
  {
    ...
    for(vector<LineSegment>::const_iterator i = singleSegs.begin(); i != singleSegs.end(); ++i)
...
{
  list<LinePercept::Line>::const_iterator closestLine = lines.end();
  int minDist = -1;
  for(list<LinePercept::Line>::const_iterator l1 = lines.begin(); l1 != lines.end(); l1++)
  {
    const int dist = (int)abs(l1->calculateDistToLine(point));
...
void LinePercept::drawOnField(const FieldDimensions& theFieldDimensions, int circleBiggerThanSpecified) const
{
  ...
  {
    for(list<Line>::const_iterator line = lines.begin(); line != lines.end(); line++)
    {
    ...
    for(list<LineSegment>::const_iterator seg = singleSegs.begin(); seg != singleSegs.end(); seg++)
...
void LinePercept::drawIn3D(const FieldDimensions& theFieldDimensions, int circleBiggerThanSpecified) const
{
  ...
  COMPLEX_DRAWING3D("representation:LinePercept",
  {
    ...
    if(circle.found)
    {
      Vector2<> v1(float(circle.pos.x + theFieldDimensions.centerCircleRadius + circleBiggerThanSpecified), (float) circle.pos.y);
      for(int i = 1; i < 33; ++i)
      {
