...
void RegionPercept::draw() const
{
  ...
  COMPLEX_DRAWING("representation:RegionPercept:FieldBorders",
  {
    ...
    {
      DOT("representation:RegionPercept:FieldBorders", i->x, i->y, ColorRGBA(255, 0, 0, 255), ColorRGBA(255, 0, 0, 255));
    }

    for(unsigned int i = 0; (i + 1) < fieldBorders.size(); i++)
    {
      LINE("representation:RegionPercept:FieldBorders", fieldBorders.at(i).x, fieldBorders.at(i).y, fieldBorders.at(i + 1).x, fieldBorders.at(i + 1).y, 0, Drawings::ps_solid, ColorRGBA(255, 0, 0, 255));
    }
  });
