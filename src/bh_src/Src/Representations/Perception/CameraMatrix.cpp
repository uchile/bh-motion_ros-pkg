...
void RobotCameraMatrix::draw()
{
  ...
  COMPLEX_DRAWING("representation:RobotCameraMatrix:Field",
  {
    ...
    if(Geometry::getIntersectionPointsOfLineAndRectangle(Vector2<int>(0, 0),
                                                         Vector2<int>(cameraInfo.resolutionWidth - 1,
                                                                      cameraInfo.resolutionHeight - 1),
...
void RobotCameraMatrix::computeRobotCameraMatrix(const RobotDimensions& robotDimensions, const float headYaw, const float headPitch, const CameraCalibration& cameraCalibration, bool upperCamera)
{
  *this = RobotCameraMatrix();
  ...
  rotateZ(headYaw);
  rotateY(-headPitch);
  translate(robotDimensions.getXHeadTiltToCamera(true), 0., robotDimensions.getZHeadTiltToCamera(true));
  rotateY(robotDimensions.getHeadTiltToCameraTilt(true) + cameraCalibration.cameraTiltCorrection);
  rotateX(cameraCalibration.cameraRollCorrection);
  rotateZ(cameraCalibration.cameraPanCorrection);
  if(upperCamera)
  {
    Pose3D upper2lower(RotationMatrix(cameraCalibration.upper2lowerRotation), cameraCalibration.upper2lowerTranslation);
    conc(upper2lower);
  }
}
