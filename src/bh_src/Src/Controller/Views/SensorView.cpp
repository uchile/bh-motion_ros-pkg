...
public:
    ...
    sensorView(sensorView), headerView(headerView), noPen(Qt::NoPen)
  {
    ...
    for(int j = 0; j < 2; ++j)
      ...
      {
        usLDistances[j][i] = SensorData::off;
        usRDistances[j][i] = SensorData::off;
      }
  ...
  void paintEvent(QPaintEvent* event)
  {
    ...
    paintRectField2 = QRect(headerView->sectionViewportPosition(2) + textOffset, 0, headerView->sectionSize(2) - textOffset * 2, lineSpacing);
    {
      ...
      if(sensorData.data[SensorData::usL] != SensorData::off)
        usLDistances[0][sensorData.usActuatorMode] = sensorData.data[SensorData::usL];
      if(sensorData.data[SensorData::usR] != SensorData::off)
        usRDistances[0][sensorData.usActuatorMode] = sensorData.data[SensorData::usR];
      if(filteredSensorData.data[SensorData::usL] != SensorData::off)
        usLDistances[1][sensorData.usActuatorMode] = filteredSensorData.data[SensorData::usL];
      if(filteredSensorData.data[SensorData::usR] != SensorData::off)
        usRDistances[1][sensorData.usActuatorMode] = filteredSensorData.data[SensorData::usR];
...
private:
  ...
  float usLDistances[2][SensorData::numOfUsActuatorModes];
  float usRDistances[2][SensorData::numOfUsActuatorModes];
