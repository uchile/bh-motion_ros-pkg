...
public:
  ...
  void paintEvent(QPaintEvent* event)
  {
    ...
    ReleaseOptions& releaseOptions = TeamComm3DCtrl::controller->releaseOptions;
    ...
    for(std::map<unsigned int, TeamComm3DCtrl::RobotData>::iterator iter = teamListener.robotData.begin(), end = teamListener.robotData.end(); iter != end; ++iter)
    {
      ...
      if(robotData.robotNumber != 0)
      {
        if(!robotData.robotHealth.robotName.empty())
        {
          ...
          print("Motion, Cognition Rate", formatedValue, std::min(computeRegularity(robotData.robotHealth.cognitionFrameRate, 29.f, 28.f), computeRegularity(robotData.robotHealth.motionFrameRate, 99.f, 98.f)));
          newBlock();
        ...
        sprintf(formatedValue, "%s%s, %s", BehaviorData::getName(robotData.behaviorData.role), robotData.isPenalized ? " (Penalized)" : "", BehaviorData::getName(robotData.behaviorData.action));
        print("Behavior Role, Action", formatedValue, 1.f);
        //print("Behavior Action", BehaviorData::getActionName(robotData.behaviorData.action), 1.f);
        newBlock();

        if(releaseOptions.motionRequest)
        {
          robotData.motionRequest.printOut(formatedValue);
          print("Motion Request", formatedValue, 1.f);
          newBlock();
        }
      }
      newSection();
...
private:
  TeamComm3DView& view;
  unsigned int lastSBSL2DebugInfoTimeStamp; /**< Timestamp of the last painted info. */
