...
void ViewBikeGLWidget::initializeGL()
{
  ...
  renderer.setSurfaceShadeMode(SimRobotCore2::Renderer::SMOOTH);
  ...
  for(int phaseNumber = 0; phaseNumber < parameters.numberOfPhases; phaseNumber++)
  {
       ...
       && !moveViewOfViewDragXP && !moveViewOfViewDragYP && !moveViewOfViewDragZP)
    {
      if(widget.selectedPoint.phaseNumber > -1 && widget.selectedPoint.limb > -1)
      {
        ...
        Vector3<> p = parameters.getPositionBlame(0.0, widget.selectedPoint.phaseNumber, widget.selectedPoint.limb);
...
{
  const float* r = viewBike.robot->getRotation();
  originRot = RotationMatrix(Matrix3x3<>(r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8]));
  ...
  if(widget.tra2dWindows || widget.tra1dWindows || widget.velocityWindows || widget.accelWindows)
  {
    ...
    if(widget.selectedPoint.phaseNumber > -1 && widget.selectedPoint.limb > -1)
    {
      Vector3<> p = parameters.getPositionBlame(0.f, widget.selectedPoint.phaseNumber, widget.selectedPoint.limb);
...
void ViewBikeGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  case Qt::Key_Right:
    ...
    break;
  case Qt::Key_X:
    widget.setDragPlane((int) SimRobotCore2::Renderer::YZ_PLANE);
    break;
  case Qt::Key_Y:
    widget.setDragPlane((int) SimRobotCore2::Renderer::XZ_PLANE);
    break;
  case Qt::Key_Z:
    widget.setDragPlane((int) SimRobotCore2::Renderer::XY_PLANE);
    break;
...
void ViewBikeGLWidget::mouseMoveEvent(QMouseEvent* event)
{
      ...
      moveViewOfViewDragYP || moveViewOfViewDragZP) && widget.selectedPoint.limb > -1)
  {
    ...
    Vector3<> oldPoint =  parameters.phaseParameters[widget.selectedPoint.phaseNumber].controlPoints[widget.selectedPoint.limb][widget.selectedPoint.pointNumber];
...
void ViewBikeGLWidget::mousePressEvent(QMouseEvent* event)
{
  ...
  if(event->button() == Qt::LeftButton)
  {
    ...
    if(renderer.startDrag(event->x(), event->y(), QApplication::keyboardModifiers() & Qt::ShiftModifier ? SimRobotCore2::Renderer::DRAG_ROTATE : SimRobotCore2::Renderer::DRAG_NORMAL))
    {
      event->accept();
      updateGL();
    }
