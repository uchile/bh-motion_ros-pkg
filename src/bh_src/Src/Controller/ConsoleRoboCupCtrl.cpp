  ...
  realtime(false)
{
  ...
  representationToFile["representation:BehaviorConfiguration"] = "behavior.cfg";
  representationToFile["representation:CameraCalibration"] = "cameraCalibration.cfg";
  representationToFile["representation:CameraSettings"] = "camera.cfg";
  representationToFile["representation:JointCalibration"] = "jointCalibration.cfg";
  representationToFile["representation:MassCalibration"] = "masses.cfg";
...
bool ConsoleRoboCupCtrl::compile()
{
  ...
  for(std::list<RemoteRobotWithPuppet*>::iterator i = remoteRobotsWithPuppet.begin(); i != remoteRobotsWithPuppet.end(); ++i)
    (*i)->handleConsole("endOfStartScript");
...
void ConsoleRoboCupCtrl::executeConsoleCommand(std::string command, RobotConsole* console)
{
  ...
  else if(buffer == "scwp")
  {
    ...
    if(!startRemoteRobotWithPuppet(stream))
    {
      selected.clear();
      if(!robots.empty())
        selected.push_back((*robots.begin())->getRobotProcess());
...
void ConsoleRoboCupCtrl::help(InConfigMemory& stream)
{
  ...
  list("  scwp <name> [<a.b.c.d>] : Starts a TCP connection to a remote robot with a robot puppet shown locally.", pattern, true);
  ...
  list("  bb off | on : Switching putting the ball back on or off.", pattern, true);
  ...
  list("  ci off | on [<fps>]: Switch the calculation of images on or off.", pattern, true);
  list("  ct off | on | undo | <color> [ replace | add | <hmin> <hmax> <smin> <smax> <imin> <imax>] | clear [ <color> ] | shrink [ <color> ] | grow [ <color> ] | imageRadius <number> | colorSpaceRadius <number> | smart [off] | mask <color>: Classic methods to modify color table.", pattern, true);
  list("  ct load <file> | save <file> | send [ <ms> | off ] | send-AndWrite | hash | kd (save <file> | load <file>): Save/load color table and related training data.", pattern, true);
  list("  ct <color> | training [ on | off ] | reclassify | kd ( undo | clear [ <color> ] | replace <color> <color> | range <distance> | neighbors <count> ): Tree based methods to modify 3D-tree and color table.", pattern, true);
  ...
  list("  log start | pause | stop | forward [image] | backward [image] | repeat | goto <number> | cycle | once : Replay log file.", pattern, true);
  ...
  list("  saveImage <filename> : Save the current image as a bitmap.", pattern, true);
...
bool ConsoleRoboCupCtrl::startRemoteRobotWithPuppet(InConfigMemory& stream)
{
  std::string name,
              ip;
  stream >> name >> ip;
  ...
  RemoteRobotWithPuppet* rr = new RemoteRobotWithPuppet(name.c_str(), ip.c_str());
  this->robotName = 0;
  if(!rr->isClient())
  {
    if(ip != "")
  ...
  selected.clear();
  remoteRobotsWithPuppet.push_back(rr);
  selected.push_back(remoteRobotsWithPuppet.back());
  rr->addViews();
  rr->start();
  return true;
}
...
bool ConsoleRoboCupCtrl::startVideo(InConfigMemory& stream)
{
  std::string name,
              fileName;
  int offset;
...
bool ConsoleRoboCupCtrl::startTimeCtrl(InConfigMemory& stream)
{
...
void ConsoleRoboCupCtrl::createCompletion()
{
  ...
  for(int i = 1; i < numOfMessageIDs; ++i)
    {
      completion.insert(std::pair<std::string, int>(std::string("log keep ") + getName(MessageID(i)), 0));
      completion.insert(std::pair<std::string, int>(std::string("log remove ") + getName(MessageID(i)), 0));
    }

  for(int color = ColorClasses::none; color < ColorClasses::numOfColors; ++color)
  {
    completion.insert(std::pair<std::string, int>(std::string("ct ") + translate(ColorClasses::getName((ColorClasses::Color) color)) + " add", 0));
    completion.insert(std::pair<std::string, int>(std::string("ct ") + translate(ColorClasses::getName((ColorClasses::Color) color)) + " replace", 0));
    completion.insert(std::pair<std::string, int>(std::string("ct clear ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
    completion.insert(std::pair<std::string, int>(std::string("ct mask ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
    completion.insert(std::pair<std::string, int>(std::string("ct grow ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
    completion.insert(std::pair<std::string, int>(std::string("ct shrink ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
    completion.insert(std::pair<std::string, int>(std::string("ct kd clear ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
    completion.insert(std::pair<std::string, int>(std::string("ct kd replace ") + translate(ColorClasses::getName((ColorClasses::Color) color)), 0));

    for(int innerColor = ColorClasses::none; innerColor < ColorClasses::numOfColors; ++innerColor)
    {
      completion.insert(std::pair<std::string, int>(std::string("ct kd replace ")
  ...
  if(drawingManager)
  {
    if(imageViews)
      ...
      {
        completion.insert(std::pair<std::string, int>(std::string("cameraCalibrator ") + translate(v->first) + " on", 0));
        completion.insert(std::pair<std::string, int>(std::string("cameraCalibrator ") + translate(v->first) + " off", 0));
      }
        ...
        if(translate(debugRequestTable->debugRequests[j].description).substr(0, 5) == "plot:")
        {
          for(int color = 1; color < ColorClasses::numOfColors; ++color)
            completion.insert(std::pair<std::string, int>(std::string("vpd ") + i->first + " " +
                              ...
                              translate(ColorClasses::getName((ColorClasses::Color) color)), 0));
          completion.insert(std::pair<std::string, int>(std::string("vpd ") + i->first + " " +
  ...
  for(tr1::unordered_map<std::string, std::string>::const_iterator i = representationToFile.begin();
    ...
    completion.insert(std::pair<std::string, int>(std::string("save ") + i->first, 0));
}
...
void ConsoleRoboCupCtrl::addCompletionFiles(const std::string& command, const std::string& pattern)
{
  ...
  for(int i = 0; i < filenames.size(); i++)
  {
    if(regExp.exactMatch(filenames[i]))
    {
      ...
      completion.insert(std::pair<std::string, int>(command + filenames[i].toAscii().constData(), 0));
    }
  }
}
