...
)
{
  ...
  for(int j = y1; j < y2 - 2; j++)
  {
    for(int i = x1; i < x2 - 2; i++)
    {
      if(zComponent == -1)
      {
        unsigned char* channels = convertedImage.image[j][i].channels;
      ...
      {
        x = (float)(+i - image.cameraInfo.resolutionWidth / 2) * tmp;
        y = (float)(-j + image.cameraInfo.resolutionHeight / 2) * tmp;
        xn = (float)(+i + 1 - image.cameraInfo.resolutionWidth / 2) * tmp;
        yn = (float)(-j - 1 + image.cameraInfo.resolutionHeight / 2) * tmp;
        z00 = (float)(-0.5f + convertedImage.image[j][i].channels[zComponent] * tmp / 2);
        if(polygons)
        {
          z01 = (float)(-0.5f + convertedImage.image[j + 1][i].channels[zComponent] * tmp / 2);
          z10 = (float)(-0.5f + convertedImage.image[j][i + 1].channels[zComponent] * tmp / 2);
          z11 = (float)(-0.5f + convertedImage.image[j + 1][i + 1].channels[zComponent] * tmp / 2);
          glColor3ub(rgbImage.image[j][i].r, rgbImage.image[j][i].g, rgbImage.image[j][i].b);
          glVertex3d(x, y, z00);
