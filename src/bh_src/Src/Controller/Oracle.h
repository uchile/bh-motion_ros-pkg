...
public:
  ...
  enum BallOut { NONE, GOAL_BY_RED, GOAL_BY_BLUE, WAIT_STATE_READY, WAIT_STATE_SET, OUT_BY_RED, OUT_BY_BLUE };
  ...
  */
  ...
  /**
   * Switches the camera according to the request and updates the ImageInfo
   * afterwards.
   *
   * @param request The request to switch the camera
   * @param info The information about the the selected camera which is to be updated.
   */
  void getAndSetCamera(const ImageRequest& request, ImageInfo& info);
...
private:
  ...
  float motionCycleTime; /**< The Length of one simulation step in seconds. */
  ...
  unsigned int framesSinceCameraSwitching; /**< Image counter for simulating camera switch delay */
  ...
  SimRobot::Object* fsrSensors[4 * 2]; /**< The handles to the sensor port of the virtual foot pressure sensors */
  SimRobot::Object* fsrBumperSensors[2]; /**< The handles to simplified "fsr" bumpers */
  ...
  SimRobot::Object* cameraSelectActuator; /** The handle to make the simulated robot switch it's camera. */

  static SimRobotCore2::Sensor* activeCameras[10]; /**< An array of all activated cameras */
  ...
  unsigned int lastUsMeasurement;

  static float walkHeight; /**< The foot height threshold between walking and kicking. */
  static unsigned int lastKick; /**< Time when last robot kicked. */
  static Pose2D lastKickPose; /**< Position of last kicking robot, orientation is toward opponent goal (0/180 degress). */
  static Pose2D lastClosestPose; /**< Position of the robot closest to the ball, orientation is toward opponent goal (0/180 degress). */
  static float minBallDistance; /**< The minimum distance between player and ball. */
  ...
  * @param z The z-component of the translation that is not part of the two-dimensional position
  */
  static Vector2<> getPosition(SimRobot::Object* obj, float& z);
  ...
  * @param z The z-component of the translation that is not part of the two-dimensional pose
  */
  void getPose2D(SimRobot::Object* obj, Pose2D& pose2D, float& z) const;
