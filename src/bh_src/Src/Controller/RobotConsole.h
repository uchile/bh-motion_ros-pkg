...
protected:
  ...
  ImageInfo imageInfo; /**< The image information received from the robot code. */
...
public:
  ...
  */
  ...
  /**
  * The method tells if current robot waiting for the ready state.
  * @return is the robot waiting?
  */
  bool isWaitingForSetState();
...
private:
  ...
  */
  ...
  bool joystickMaps(InConfigMemory& stream);
  bool log(InConfigMemory& stream);
  bool get(InConfigMemory& stream, bool first, bool print);
  bool set(InConfigMemory& stream);
  bool penalizeRobot(InConfigMemory& stream);
  bool saveImage(InConfigMemory& stream);
  bool saveRequest(InConfigMemory& stream, bool first);
  bool sendMof(InConfigMemory& stream);
  bool sendWek(InConfigMemory& stream);
