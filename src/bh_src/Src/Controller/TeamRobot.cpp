...
int TeamRobot::main()
{
  ...
  int lineY = 2550;
  DRAWTEXT("robotState", -3600, lineY, 150, ColorClasses::white, "batteryLevel: " << robotHealth.batteryLevel << " %");
  DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "role: " << BehaviorData::getName(behaviorData.role));
  lineY -= 180;
  DRAWTEXT("robotState", -3600, lineY, 150, ColorClasses::white, "temperatures: joint: " << robotHealth.maxJointTemperature << " C, cpu: " << robotHealth.cpuTemperature << " C, board: " << robotHealth.boardTemperature << " C");
  DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "action: " << BehaviorData::getName(behaviorData.action));
  lineY -= 180;
  DRAWTEXT("robotState", -3600, lineY, 150, ColorClasses::white, "rates: cognition: " << roundNumberToInt(robotHealth.cognitionFrameRate) << " fps, motion: " << roundNumberToInt(robotHealth.motionFrameRate) << " fps");
  if(ballModel.timeWhenLastSeen)
  {
    DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "ballLastSeen: " << SystemCall::getRealTimeSince(ballModel.timeWhenLastSeen) << " ms");
  }
  else
  {
    DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "ballLastSeen: never");
  }
  ...
  lineY -= 180;
  DRAWTEXT("robotState", -3600, lineY, 150, ColorClasses::white, "memory usage: " << robotHealth.memoryUsage << " %");
  if(goalPercept.timeWhenOppGoalLastSeen)
  {
    DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "goalLastSeen: " << SystemCall::getRealTimeSince(goalPercept.timeWhenOppGoalLastSeen) << " ms");
  }
  else
  {
    DRAWTEXT("robotState", 1200, lineY, 150, ColorClasses::white, "goalLastSeen: never");
  }
  lineY -= 180;
  DRAWTEXT("robotState", -3600, lineY, 150, ColorClasses::white, "load average: " << (float(robotHealth.load[0]) / 10.f) << " " << (float(robotHealth.load[1]) / 10.f) << " " << (float(robotHealth.load[2]) / 10.f));
  lineY -= 180;
  DRAWTEXT("robotState", -2600, 0, 150, ColorRGBA(255, 255, 255, 50), robotHealth.robotName);
  ...
  if(SystemCall::getTimeSince(isPenalizedReceived) < 1000 && isPenalized)
  {
    ...
    DRAWTEXT("robotState", -1300, 0, 200, ColorClasses::red, "PENALIZED");
  }
