...
void WalkingEngine::init()
{
  ...
  //TODO: hacer que el stream funcione
  InConfigMap stream("/home/nao/.config/naoqi/Data/Config/walking.cfg");
  if(stream.exists())
    stream >> p;
  else
  {
...
*/
  ...
  {
    /*std::cout << theMotionRequest.walkRequest.speed.translation.x <<std::endl;
    std::cout << theMotionRequest.walkRequest.speed.translation.y <<std::endl;
    std::cout << theMotionRequest.walkRequest.target.translation.x <<std::endl;
    std::cout << theMotionRequest.walkRequest.target.translation.y <<std::endl;*/
    //std::cout << currentMotionType << std::endl;
    ...
    updateMotionRequest(theMotionRequest,theGroundContactState,theDamageConfiguration,theMotionSelection,walkingEngineOutput);
    updateObservedPendulumPlayer(theFrameInfo,theMotionRequest);
    computeMeasuredStance(theRobotModel,theRobotDimensions,theTorsoMatrix);
    computeExpectedStance();
    computeError(theGroundContactState,theDamageConfiguration,theFallDownState,theMotionSelection,theFrameInfo);
    updatePendulumPlayer(theFrameInfo,theMotionRequest);
    updateKickPlayer(theMotionRequest,theFrameInfo);
    generateTargetStance(theHeadJointRequest,theMotionRequest);
    generateJointRequest(theFrameInfo,theMassCalibration,theRobotDimensions);
    computeOdometryOffset(theTorsoMatrix,theMotionRequest);
    generateOutput(walkingEngineOutput,theMotionRequest);
  }
  else
  {
...
void WalkingEngine::updateMotionRequest(const MotionRequest& theMotionRequest
                                        ,const GroundContactState& theGroundContactState
                                        ,const DamageConfiguration& theDamageConfiguration
                                        ,const MotionSelection& theMotionSelection
...
void WalkingEngine::updateObservedPendulumPlayer(const FrameInfo& theFrameInfo
                                                 ,const MotionRequest& theMotionRequest)
{
  // motion update
  if(observedPendulumPlayer.isActive())
    observedPendulumPlayer.seek(theFrameInfo.cycleTime,theMotionRequest);
  ...
  case standRight:
    ...
    if(requestedMotionType != currentMotionType)
    {
      ...
      observedPendulumPlayer.init(stepType, p.observerMeasurementDelay * -0.001f, supportLeg, r, x0, k, theFrameInfo.cycleTime, theMotionRequest);
...
void WalkingEngine::computeMeasuredStance(const RobotModel& theRobotModel
                                          ,const RobotDimensions& theRobotDimensions
                                          ,const TorsoMatrix& theTorsoMatrix)
{
...
void WalkingEngine::computeError(const GroundContactState& theGroundContactState
                                 ,const DamageConfiguration& theDamageConfiguration
                                 ,const FallDownState& theFallDownState
                                 ,const MotionSelection& theMotionSelection
                                 ,const FrameInfo& theFrameInfo)
{
  if((theGroundContactState.contactSafe || !theDamageConfiguration.useGroundContactDetectionForSafeStates) && theFallDownState.state != FallDownState::onGround && theFallDownState.state != FallDownState::falling)
  {
    ...
    {  
      ...
      {
        //std::cout << p.stabilizerOnThreshold << std::endl;
        //std::cout << instability.getAverage()<<std::endl;
        instable = true;
        beginOfStable = 0;
      }
...
void WalkingEngine::updatePendulumPlayer(const FrameInfo& theFrameInfo
                                         ,const MotionRequest& theMotionRequest)
{
  if(currentMotionType == stepping)
  {
    ...
    */
    ...
    pendulumPlayer.seek(p.observerMeasurementDelay * 0.001f,theMotionRequest);
...
void WalkingEngine::updateKickPlayer(const MotionRequest& theMotionRequest
                                     ,const FrameInfo& theFrameInfo)
{
  if(currentMotionType == stepping)
  {
...
void WalkingEngine::generateTargetStance(const HeadJointRequest& theHeadJointRequest
                                         ,const MotionRequest& theMotionRequest)
{
...
void WalkingEngine::generateJointRequest(const FrameInfo& theFrameInfo
                                        ,const MassCalibration& theMassCalibration
                                        ,const RobotDimensions& theRobotDimensions)
{
...
void WalkingEngine::generateNextStepSize(SupportLeg nextSupportLeg,
                                         StepType lastStepType, WalkRequest::KickType lastKickType,
...
void WalkingEngine::computeOdometryOffset(const TorsoMatrix& theTorsoMatrix
                                          ,const MotionRequest& theMotionRequest)
{
...
void WalkingEngine::ObservedPendulumPlayer::init(StepType stepType, float t, SupportLeg supportLeg, const Vector2<>& r, const Vector2<>& x0, const Vector2<>& k, float deltaTime,const MotionRequest& theMotionRequest)
{
  ...
  generateNextStepSize(theMotionRequest);
...
void WalkingEngine::PendulumPlayer::seek(float deltaT,const MotionRequest& theMotionRequest)
{
  t += deltaT;
  if(t >= 0.f)
  {
...
void WalkingEngine::PendulumPlayer::generateNextStepSize(const MotionRequest& theMotionRequest)
{
  walkingEngine->generateNextStepSize(supportLeg == right ? left : right, type, kickType, next,theMotionRequest);
}
