  ...
  readOdometry(false)
{
  ...
  InConfigFile file("/home/nao/.config/naoqi/Data/Config/specialActions.dat");
  if(!file.exists() || file.eof())
  {
    std::cout << "SpecialActions : Error, 'specialActions.dat' not found." << std::endl;
    ASSERT(false);
    //OUTPUT(idText, text, "SpecialActions : Error, 'specialActions.dat' not found.");
  }
  else
  {
     std::cout << "SpecialActions : Leyendo 'specialActions.dat'." << std::endl;
...
void SpecialActions::init(const FrameInfo& theFrameInfo)
{
  // read entries from file
  InConfigMap cm("/home/nao/.config/naoqi/Data/Config/odometry.cfg");
  if(!cm.exists())
  {
    std::cout << "SpecialActions : Error, 'odometry.cfg' not found." << std::endl;
    ASSERT(false);
    //OUTPUT(idText, text, "SpecialActions : Error, 'odometry.cfg' not found.");
  }
  else
  {
    std::cout << "SpecialActions : Leyendo 'odometry.cfg' ." << std::endl;
    ...
    for(std::vector<SpecialActionInfo>::const_iterator it = infos.specialActionInfos.begin(); it != infos.specialActionInfos.end(); it++)
    {
      ...
      {
        infoTable[it->id].odometryOffset.fromAngle(it->odometryOffset.rotation);
...
void SpecialActions::calculateJointRequest(JointRequest& jointRequest, const FilteredJointData& theFilteredJointData, const HardnessSettings theHardnessSettings)
{
  float ratio, f, t;
...
void SpecialActions::update(SpecialActionsOutput& specialActionsOutput, const MotionSelection theMotionSelection, const FilteredJointData& theFilteredJointData, const HardnessSettings theHardnessSettings, const FrameInfo& theFrameInfo)
{
  if(!readOdometry)
  {
        init(theFrameInfo);
  ...
  if(theMotionSelection.specialActionMode != MotionSelection::deactive)
  {
    ...
    calculateJointRequest(specialActionsOutput,theFilteredJointData,theHardnessSettings);
