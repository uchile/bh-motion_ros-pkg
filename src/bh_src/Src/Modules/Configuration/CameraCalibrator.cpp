...
CameraCalibrator::CameraCalibrator(UChBlackBoard *blackboard) : calibrationState(Idle), lastFetchedPoint(-1, -1), optimizer(NULL),bb(blackboard)
{
}
...
void CameraCalibrator::update(CameraCalibration& cameraCalibration)
{
  ...
  //MODIFY("module:CameraCalibratior2:parameters", parameters);
  ...
  */
  ...
  case(Optimize): // optimize the parameters of the cameraCalibration
    if(!optimizer)
    {
      ...
      initialParameters.resize(parameters.calibrateBothCameras ? numOfParameterTranslations : numOfParametersLowerCamera);
    ...
    else
    {
      ...
      if(framesToWait <= 0)
      {
        framesToWait = parameters.framesToWait;
        ...
        if(abs(delta) < parameters.terminationCriterion)
        {
          ++successiveConvergations;
        }
        if(successiveConvergations >= parameters.successiveConvergations)
        {
  ...
  //{
  //  DRAWTEXT("module:CameraCalibrator:points", 10, 10, 250, !(samples.size() > numOfParameterTranslations) ? ColorClasses::red : ColorClasses::green, "Points collected: " << (unsigned) samples.size());
  //});
}
...
float CameraCalibrator::computeError(const Sample& sample, const CameraCalibration& cameraCalibration, const RobotPose& robotPose, bool inImage) const
{
  ...
  if(!parameters.calibrateBothCameras && sample.upperCamera)
  {
    return 0.0f;
  }

  // build camera matrix from sample and camera calibration
  const RobotCameraMatrix robotCameraMatrix(bb->theRobotDimensions, sample.headYaw, sample.headPitch, cameraCalibration, sample.upperCamera);
  ...
  if(inImage)
  {
    ...
    for(vector<FieldDimensions::LinesTable::Line>::const_iterator i = bb->theFieldDimensions.fieldLines.lines.begin(); i != bb->theFieldDimensions.fieldLines.lines.end(); ++i)
    {
      ...
      {
        distance = parameters.aboveHorizonError;
      }
      else
      {
        distance = Geometry::getDistanceToEdge(lineInImage, Vector2<>((float)sample.pointInImage.x, (float)sample.pointInImage.y));
      }
      if(distance < minimum)
      {
        minimum = distance;
  ...
  else // on ground
  {
    ...
    Vector3<> cameraRay(bb->theCameraInfo.focalLength, bb->theCameraInfo.opticalCenter.x - sample.pointInImage.x, bb->theCameraInfo.opticalCenter.y - sample.pointInImage.y);
    cameraRay = cameraMatrix * cameraRay;
    if(cameraRay.z >= 0) // above horizon
    {
      return parameters.aboveHorizonError;
    }
...
float CameraCalibrator::computeErrorParameterVector(const Sample& sample, const vector<float>& parameters) const
{
  ...
  robotPose.translation += bb->theRobotPose.translation;
  robotPose.rotation += bb->theRobotPose.rotation;
...
void CameraCalibrator::translateParameters(const vector<float>& parameters, CameraCalibration& cameraCalibration, RobotPose& robotPose) const
{
  ...
  {
    const Vector3<> axis(parameters[upperCameraX], parameters[upperCameraY], parameters[upperCameraZ]);
    cameraCalibration.upper2lowerRotation = axis;
  }
}
...
void CameraCalibrator::translateParameters(const CameraCalibration& cameraCalibration, const RobotPose& robotPose, vector<float>& parameters) const
{
  ...
  if(parameters.size() == numOfParameterTranslations)
  {
    const Vector3<>& axis(cameraCalibration.upper2lowerRotation);
    parameters[upperCameraX] = axis.x;
    parameters[upperCameraY] = axis.y;
    parameters[upperCameraZ] = axis.z;
  }
}
...
void CameraCalibrator::fetchPoint()
{
  ...
  sample.torsoMatrix = bb->theTorsoMatrix;
  sample.headYaw = bb->theFilteredJointData.angles[JointData::HeadYaw];
  sample.headPitch = bb->theFilteredJointData.angles[JointData::HeadPitch];
  sample.upperCamera = bb->theImageInfo.camera == ImageInfo::upperCamera;
...
{
  const float& f = bb->theCameraInfo.focalLength;
  ...
  const bool p1Behind = p1Camera.x < bb->theCameraInfo.focalLength;
  const bool p2Behind = p2Camera.x < bb->theCameraInfo.focalLength;
  ...
  const Vector2<> p1Result(bb->theCameraInfo.opticalCenter.x - p1Camera.y, bb->theCameraInfo.opticalCenter.y - p1Camera.z);
  const Vector2<> p2Result(bb->theCameraInfo.opticalCenter.x - p2Camera.y, bb->theCameraInfo.opticalCenter.y - p2Camera.z);
...
*/
//MAKE_MODULE(CameraCalibrator, Infrastructure)
