...
void WhiteCorrection::getAverageYCbCrMMX(Image::Pixel& color)
{
  ...
  const int width = theImage.cameraInfo.resolutionWidth;
  const int height = theImage.cameraInfo.resolutionHeight;
  ...
  int y = _mm_cvtsi64_si32(accLower); // ectract first value from MMX-register
  accLower = _mm_srli_si64(accLower, 32); // shift upper 32 bit to lower 32 bit
  int cb = _mm_cvtsi64_si32(accLower); // extract second value from MMX-register
  int yCbCrPadding = _mm_cvtsi64_si32(accUpper); // extract third value from MMX-register
  accUpper = _mm_srli_si64(accUpper, 32); // shift upper 32 bit to lower 32 bit
  int cr = _mm_cvtsi64_si32(accUpper); // extract fourth value from MMX-register
  ...
  color.y = y;
  color.cb = cb;
  color.yCbCrPadding = yCbCrPadding;
  color.cr = cr;
...
void WhiteCorrection::getAverageYCbCr(Image::Pixel& color)
{
  int y(0), cb(0), cr(0);
  // iterate all rows
  for(int yy = 0; yy < theImage.cameraInfo.resolutionHeight; yy++)
  {
    Image::Pixel* p = theImage.image[yy];
    Image::Pixel* pEnd = p + theImage.cameraInfo.resolutionWidth;
  ...
  const int size = theImage.cameraInfo.resolutionHeight * theImage.cameraInfo.resolutionWidth;
  y /= size;
  cb /= size;
  cr /= size;
...
void WhiteCorrection::update(CameraSettings& cameraSettings, const Image& theImage_)
{
    theImage = theImage_;
  ...
  case(Wait):
    if(framesWaited == 0) // before actual waiting set new camera settings
    {
      ...
      // correct redness and blueness proportional to their difference to green
      cameraSettings.blue.value -= (diffB + sgn(diffB)) / 2;
      cameraSettings.red.value -= (diffR + sgn(diffR)) / 2;
    }
