...
void TeamMarkerPerceptor::init(const Image& theImage)
{
  ...
  above.clear();
  above.reserve(theImage.cameraInfo.resolutionWidth / scanlineDiff);
  below.clear();
  below.reserve(theImage.cameraInfo.resolutionWidth / scanlineDiff);
  cbStart = 0;
  crStart = 0;
  width = 0.0;
  scanlineSegments.clear();
  scanlineSegments.reserve(theImage.cameraInfo.resolutionWidth / scanlineDiff);
  polygon.clear();
  polygon.reserve(2 * theImage.cameraInfo.resolutionWidth / scanlineDiff);
}

void TeamMarkerPerceptor::update(TeamMarkerSpots& teamMarkerSpots, const RegionPercept& theRegionPercept, const Image& theImage)
{
  /*
  ...
  */
  ...
  extractTeamMarkers(teamMarkerSpots, theRegionPercept, theImage);
}
                                             ...
                                             const Image& theImage)
{
  ...
  for(R region = theRegionPercept.regions, end = region + theRegionPercept.regionsCounter; region < end; ++region)
  {
    ...
    case ColorClasses::red:
    {
      ...
      //ABORTABLE_STEP(regionCheck(region, theImage), *region->childs[0], ColorClasses::red)
      //ABORTABLE_STEP(regionGrowing(teamMarker, theImage), approxCog, ColorClasses::green)
                                      ...
                                      const Image& theImage)
{
  ...
  if(!POINT_IS_WITHIN_IMAGE(approxCog, theImage.cameraInfo))
    return false;
                                        ...
                                        const Image& theImage)
{
  ...
  //ASSERT_POINT_WITHIN_IMAGE(approxCog, theImage.cameraInfo);
  ...
  scanDistance(theImage, Vector2<int>(-1, 0), approxCog, current);
  if(!POINT_IS_WITHIN_IMAGE(current, theImage.cameraInfo))
    return false;
  //ASSERT_COORDINATES_WITHIN_IMAGE(current.x, current.y, theImage.cameraInfo);
  ...
  if(xLength <= 3 || xLength > theImage.cameraInfo.resolutionWidth / 3)
    return false;
  ...
  for(int x = teamMarker.minX + 1; x < teamMarker.maxX; x += scanlineDiff)
  {
    ...
    current = start;
    scanDistance(theImage, Vector2<int>(0, -1), start, current, &above);
    if(!POINT_IS_WITHIN_IMAGE(current, theImage.cameraInfo))
      outsideImage++;
    ...
    current = start;
    scanDistance(theImage, Vector2<int>(0, 1), start, current, &below);
    if(!POINT_IS_WITHIN_IMAGE(current, theImage.cameraInfo))
      outsideImage++;
...
void TeamMarkerPerceptor::scanDistance(const Image& theImage,
                                       Vector2<int> dir, const Vector2<int>& start,
