...
public:
  ...
  /**
   * Returns the ratio of a given color on a defined image area.
   * @param col the color
   * @param area a area on the image
   * @param stepSize The step size in which the area is searched
   * @return the percentage [0, 1] of the color on the image area
   */
  float colorRatio(const ColorClasses::Color& col, const Boundary<int>& area, int stepSize = 2);
