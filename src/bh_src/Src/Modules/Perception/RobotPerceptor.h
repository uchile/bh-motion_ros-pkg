...
class RobotPerceptor //: public RobotPerceptorBase
{
  ...
  typedef std::vector<TeamMarkerSpots::TeamMarkerSpot>::const_iterator TMSCI;
   ...
   * @param tms A constant iterator pointing to the team marker to be checked.
   * @param lowestPx The lowest pixel of the robot will be set if the check is successful.
   * @return Is this a robot?
   */
  bool checkTeamMarkerEnvironment(TMSCI tms, Vector2<int>& lowestPx,
                                  const Image& theImage);
...
public:
  ...
  RobotPerceptor(const Image& theImage, const ColorTable64& theColorTable64);
};
