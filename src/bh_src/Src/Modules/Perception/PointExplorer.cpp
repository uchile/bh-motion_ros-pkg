...
{
  //ASSERT_COORDINATES_WITHIN_IMAGE(x, y, theImage->cameraInfo);
...
float PointExplorer::colorRatio(const ColorClasses::Color& col, const Boundary<int>& area, int stepSize)
{
  ...
  const int xStart(std::max(area.x.min, 0)),
        xEnd(std::min(area.x.max, theImage->cameraInfo.resolutionWidth - 1));
  const int yStart(std::max(area.y.min, 0)),
        yEnd(std::min(area.y.max, theImage->cameraInfo.resolutionHeight - 1));
