...
class TeamMarkerPerceptor //: public TeamMarkerPerceptorBase
{
  ...
  void init(const Image& theImage);

  public:
  void update(TeamMarkerSpots& teamMarkerSpots, const RegionPercept& theRegionPercept, const Image& theImage);
  ...
  void scanDistance(const Image& theImage, Vector2<int> dir, const Vector2<int>& start, Vector2<int>& current, Polygon* p = 0);
