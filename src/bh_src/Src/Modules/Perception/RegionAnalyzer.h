...
public:
  ...
  void update(LineSpots& lineSpots,
              const CameraMatrix& theCameraMatrix,
              const RegionPercept& theRegionPercept) {lineSpots = this->lineSpots;}

private:
  /** analyzes all the regions and create the ballSpots, lineSpots and nonLineSpots */
  void analyzeRegions(BallSpots& ballSpots, const RegionPercept& theRegionPercept);
   ...
   * @return is it a line?
   */
  inline bool isLine(const RegionPercept::Region* region, float& direction, LineSpots::LineSpot& spot, const RegionPercept& theRegionPercept);
