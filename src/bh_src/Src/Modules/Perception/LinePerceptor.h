...
class LinePerceptor // : public LinePerceptorBase
{
   ...
   */
  class ParameterWrapper : public Streamable
  {
  private:
    void serialize(In* in, Out* out)
    {
      ...
      STREAM(parameters);
      STREAM(circleParams);
      STREAM(nonLineParams);
      STREAM(banSectorParams);
      STREAM_REGISTER_FINISH();
    }
  ...
  private:
  /** create BanSectors */
  void createBanSectors(const LineSpots& theLineSpots,
                        const ImageCoordinateSystem& theImageCoordinateSystem,
                        const CameraMatrix& theCameraMatrix,
                        const CameraInfo& theCameraInfo);
