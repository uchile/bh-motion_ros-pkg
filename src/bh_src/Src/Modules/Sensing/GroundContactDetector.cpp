...
void GroundContactDetector::init()
{
  ...
  InConfigMap stream("/home/nao/.config/naoqi/Data/Config/Robots/Default/groundContact.cfg");
  ...
  /*p.forceContact = false;
  p.fsrLimit = 3.5;
  p.fsrThreshold = 0.5;
  p.loadThreshold = 550;
  p.contactThreshold = 1.5;
  p.noContactThreshold = 2;
  p.safeContactTime = 3300;
  p.safeNoContactTime = 1000;*/
}

struct GroundContactDetector::ContactState GroundContactDetector::checkFsr(bool left,const SensorData& theSensorData)
{
  ContactState state;
  float fsrValues[4] =
  {
