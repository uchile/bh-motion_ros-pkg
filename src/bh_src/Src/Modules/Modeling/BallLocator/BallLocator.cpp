...
BallLocator::BallLocator(UChBlackBoard* blackboard)
    :bb(blackboard)
{
    init();
}
...
{
  p.processDeviation = Vector4f(0.1f, 0.1f, 1.f, 1.f);
  p.robotRotationDeviation = Vector2<>(0.02f, 0.08f);
  p.odometryDeviation = Pose2D(0.5f, 0.5f, 0.5f);

  p.initialStateWeight = 0.1f;

  p.footOffset = Vector2<>(65.f, 0.f);
  p.footRadius = 50.f;
  p.footMass = 0.1f;
  p.ballMass = 0.05f;
  p.kickDeviation = Vector2<>(1.f, 1.f);

  lastOdometryData = bb->theOdometryData;
  lastFrameTime = bb->theFrameInfo.time;
...
{
  //MODIFY("module:BallLocator:parameters", p);
...
#ifdef TARGET_SIM
  if(bb->theFrameInfo.time <= lastFrameTime)
  {
    if(bb->theFrameInfo.time < lastFrameTime)
      init();
    else
  ...
  Pose3D leftFoot = bb->theTorsoMatrix;
  Pose3D rightFoot = bb->theTorsoMatrix;
  leftFoot.conc(bb->theRobotModel.limbs[MassCalibration::footLeft]).translate(p.footOffset.x, p.footOffset.y, -bb->theRobotDimensions.heightLeg5Joint);
  rightFoot.conc(bb->theRobotModel.limbs[MassCalibration::footRight]).translate(p.footOffset.x, -p.footOffset.y, -bb->theRobotDimensions.heightLeg5Joint);
  ...
  /*COMPLEX_DRAWING("module:BallLocator:Image",
  {
    ...
    Geometry::calculatePointInImage(Vector3<>(leftFootCenter.x + p.footRadius, leftFootCenter.y, 0), theCameraMatrix, theCameraInfo, leftTopImage))
    {
    ...
    Geometry::calculatePointInImage(Vector3<>(rightFootCenter.x + p.footRadius, rightFootCenter.y, 0), theCameraMatrix, theCameraInfo, rightTopImage))
    {
  ...
  // sensor update step
  const Vector2f ballPercept(bb->theBallPercept.relativePositionOnField.x, bb->theBallPercept.relativePositionOnField.y);
  Matrix2x2f ballPerceptCov;
  if(bb->theBallPercept.ballWasSeen)
  {
    // reset filters..
    if(bb->theFrameInfo.getTimeSince(bb->theBallHypotheses.timeWhenDisappeared) >= 4000)
      reset();

    // calculate variance of the percept
    ballPerceptCov = getCovOfPixelInWorld(bb->theImageCoordinateSystem.toCorrected(bb->theBallPercept.positionInImage), float(bb->theFieldDimensions.ballRadius));
  ...
  if(bb->theBallPercept.ballWasSeen)
  {
    ...
    lastBallPerceptTimeStamp = bb->theFrameInfo.time;
    lastBallPerceptCov = ballPerceptCov;
    hasLastBallPercept = true;
  }

  //
  //TEAM_OUTPUT_FAST(idTeamMateBallModel, bin, BallModelCompressed(ballModel));
}
...
void BallLocator::motionUpdate(BallModel& ballModel)
{
  ...
  float deltaTime = (bb->theFrameInfo.time - lastFrameTime) * 0.001f; // in seconds
  float deltaTimeInv = 1.f / deltaTime;
  ...
  Vector2f odometryTranslationDeviation(odometryOffset.translation.x * p.odometryDeviation.translation.x, odometryOffset.translation.y * p.odometryDeviation.translation.y);
  Vector4f odometryTranslationCov(sqr(odometryTranslationDeviation.x), sqr(odometryTranslationDeviation.y),
                                  sqr(odometryTranslationDeviation.x * deltaTimeInv), sqr(odometryTranslationDeviation.y * deltaTimeInv));
  ...
  float deceleration = float(bb->theFieldDimensions.ballFriction) * deltaTime;
  float sqrDeceleration = sqr(deceleration);

  // prepare process noise
  Vector4f processCov(sqr(p.processDeviation[0]), sqr(p.processDeviation[1]), sqr(p.processDeviation[2]), sqr(p.processDeviation[3]));
  ...
  if(!bb->theBallPercept.ballWasSeen)
  {
    Vector2f lastPercept = fixedOdometryRotation * Vector2f(ballModel.lastPerception.position.x, ballModel.lastPerception.position.y) + fixedOdometryTranslation;
    ballModel.lastPerception.position = Vector2<>(lastPercept.x, lastPercept.y);
    Vector2f lastSeenEstimate = fixedOdometryRotation * Vector2f(ballModel.lastSeenEstimate.position.x, ballModel.lastSeenEstimate.position.y) + fixedOdometryTranslation;
    ballModel.lastSeenEstimate.position = Vector2<>(lastSeenEstimate.x, lastSeenEstimate.y);
    Vector2f lastSeenEstimateVelocity = fixedOdometryRotation * Vector2f(ballModel.lastSeenEstimate.velocity.x, ballModel.lastSeenEstimate.velocity.y);
    ballModel.lastSeenEstimate.velocity = Vector2<>(lastSeenEstimateVelocity.x, lastSeenEstimateVelocity.y);
  }
  ...
  for(State* state = states, * end = states + stateCount; state < end; ++state)
  {
    ...
    if(state->type == State::moving)
    {
      ...
      float sqrVelocity =  velocity.sqr();
      if(sqrVelocity <= sqrDeceleration)
      {
      ...
      {
        float absVelocity = sqrt(sqrVelocity);
        velocity *= (absVelocity - deceleration) / absVelocity; // <=> velocity.normalize(velocity.abs() - deceleration)
        x[2] = velocity.x;
        x[3] = velocity.y;
      }
    }
...
void BallLocator::handleCollisionWithFeet(const Vector2<>& leftFootCenter, const Vector2<>& rightFootCenter)
{
  ...
  float assumedRadius = p.footRadius + bb->theFieldDimensions.ballRadius;
  ...
  if(leftCollisionFactor < 0.f || leftCollisionFactor > 1.f)
    leftCollision = false;
  ...
  if(rightCollisionFactor < 0.f || rightCollisionFactor > 1.f)
  ...
  if(leftCollision || rightCollision)
  {
    ...
    float deltaTime = (bb->theFrameInfo.time - lastFrameTime) * 0.001f;
    float footMomentum = (footCenter - lastFootCenter).abs() / deltaTime * p.footMass;
    float passedMomentum = footMomentum * cos(abs(normalize(collisionAngle - (footCenter - lastFootCenter).angle())));
    // TODO: calculate reflected momentum?
    if(passedMomentum > 0.f)
    {
      ballVelocity = (collisionPoint - lastFootCenter).normalize(passedMomentum) / p.ballMass;
  ...
  else
  {
    ...
    Vector2<> leftToRight = rightFootCenter - leftFootCenter;
  ...
  if(leftCollision || rightCollision || centerCollision || ballShift != Vector2<>())
  {
    if(ballVelocity != Vector2<>())
    {
      ...
      cov[2][2] += sqr(ballVelocity.x * p.kickDeviation.x);
      cov[3][3] += sqr(ballVelocity.y * p.kickDeviation.y);
    }
    else
    {
...
// if(bestState && bestState->type == State::moving)
// {
  // int k = 23;
// }
}
...
void BallLocator::createNewStates(const Vector2f& ballPercept, const Matrix2x2f& ballPerceptCov, State* worstStationaryState, State* worstMovingState)
{
  ...
  ASSERT(stateCount > 0);
  newState->weight = p.initialStateWeight;
  newState->age = 2;
  ...
  if(hasLastBallPercept)
  {
    ...
    newState->weight = p.initialStateWeight;
    newState->age = 0;
    float timeScale = 1.f / ((bb->theFrameInfo.time - lastBallPerceptTimeStamp) * 0.001f);
...
void BallLocator::generateModel(BallModel& ballModel)
{
  ...
  if(bestState)
  {
    ...
    if(bestState->type == State::moving)
    {
      ...
      ballModel.endPosition = ballState.position + Vector2<>(ballState.velocity).normalize(ballState.velocity.squareAbs() / (2.f * (float) bb->theFieldDimensions.ballFriction));
    ...
    else
    {
      ...
      ballModel.endPosition = ballState.position;
      //PLOT("module:BallLocator:variance", sqrt(bestState->stationaryCov[0][0] + bestState->stationaryCov[1][1]));
    }
  }
  if(bb->theBallPercept.ballWasSeen)
  {
    ballModel.timeWhenLastSeen = bb->theFrameInfo.time;
    ballModel.lastPerception.position = bb->theBallPercept.relativePositionOnField;
    ballModel.lastPerception.velocity = Vector2<>();
...
{
  const Vector3<> unscaledVectorToPoint(bb->theCameraInfo.focalLength, bb->theCameraInfo.opticalCenter.x - correctedPointInImage.x, bb->theCameraInfo.opticalCenter.y - correctedPointInImage.y);
  ...
  Matrix2x2f cov(Vector2f(sqr(h / tan((distance == 0.f ? pi_2 : atan(h / distance)) - p.robotRotationDeviation.x) - distance), 0.f),
                 Vector2f(0.f, sqr(tan(p.robotRotationDeviation.y) * distance)));
...
float BallLocator::getUnscaledProbabilityAt(const Vector2f& mean, const Matrix2x2f& cov, const Vector2f& pos) const
{
  ...
  float p = exp(-0.5f * exponent);
  return max(p, 0.01f /*0.0000001f*/);
}

float BallLocator::getProbabilityAtMean(const Matrix2x2f& cov) const
{
  return 1.f / max((pi2 * sqrt(max(cov.det(), 0.f))), 0.0000001f);
}
