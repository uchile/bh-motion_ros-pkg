...
FieldCoverageProvider::FieldCoverageProvider(UChBlackBoard* bb)
    : range(2000), lastWorstIdx(-1), lastWorstNoTurnIdx(-1), lastWorstNoTurnRangeIdx(-1), lastWorstNoTurnHalfRangeIdx(-1),
    visibleAngle(fromDegrees(20.0f)), throwInLineDistanceSL(400.0f), throwInLineDistanceGL(1000.f),
    throwInLineOffset(0), lastThrowInTimestamp(0), throwInPenaltyDistance(1000.f), throwInCellDiff(10),
    minThrowInCellCoverage((xSteps - 1) * throwInCellDiff), lastGameState(STATE_INITIAL),blackboard(bb)
{
    init();
}

void FieldCoverageProvider::init()
{
  const float halfFieldLength = (float) blackboard->theFieldDimensions.xPosOpponentGroundline;
  const float halfFieldWidth = (float) blackboard->theFieldDimensions.yPosLeftSideline;
  ...
  lastGameState = blackboard->theGameInfo.state;
  ...
  std::vector<FieldDimensions::LinesTable::Line>::const_iterator it = blackboard->theFieldDimensions.fieldBorder.lines.begin();
  std::vector<FieldDimensions::LinesTable::Line>::const_iterator end = blackboard->theFieldDimensions.fieldBorder.lines.end();
  for(; it != end; ++it)
...
void FieldCoverageProvider::update(FieldCoverage& fieldCoverage)
{
  ...
  if(blackboard->theCombinedWorldModel.ballIsValid)
  {
    if(blackboard->theFieldDimensions.isInsideField(blackboard->theCombinedWorldModel.ballState.position))
    {
      lastValidInsideBall.position = blackboard->theCombinedWorldModel.ballState.position;
      lastValidInsideBall.time = blackboard->theFrameInfo.time;
      if(blackboard->theCombinedWorldModel.ballState.velocity.abs() < 1.f)
        lastValidInsideLyingBall = lastValidInsideBall;
    }
    else
    {
      lastValidOutsideBall.position = blackboard->theCombinedWorldModel.ballState.position;
      lastValidOutsideBall.time = blackboard->theFrameInfo.time;
    }
  }

  // Precompute relative targets and distances.
  const Pose2D robotPoseInv = blackboard->theRobotPose.invert();
  for(size_t x = 0; x < xSteps; ++x)
    for(size_t y = 0; y < ySteps; ++y)
    {
  ...
  if(blackboard->theCameraMatrix.isValid && blackboard->theRobotPose.deviation < 50.0f)
  {

    const int xMax = blackboard->theCameraInfo.resolutionWidth - 1;
    const int yMin = (int)std::max(blackboard->theImageCoordinateSystem.origin.y + blackboard->theImageCoordinateSystem.rotation.c[1].y * 25, 0.f);
    const int yMax = blackboard->theCameraInfo.resolutionHeight - 1;
    ...
    computeShadows(shadows, blackboard->theRobotPose, blackboard->theRobotsModel);
    ...
    for(size_t x = 0; x < xSteps; x++)
    {
      for(size_t y = 0; y < ySteps; y++)
      {
        ...
        if(Geometry::isPointInsideConvexPolygon(cameraRange, 4, c.absoluteCameraTargetOnField))
        {
          ...
          {
            c.shadowed = false;
            c.lastseenScanPattern = blackboard->theFrameInfo.time;
            if(c.distance < range)
            {
              c.lastseen = blackboard->theFrameInfo.time;
              fieldCoverage.cells[x * ySteps + y] = blackboard->theFrameInfo.time;
            }
          }
        }
      }
  ...
  // Set cells which are cut by blackboard->the throw-in lines to be uncovered if the ball is thrown in.
  ...
  unsigned int worstNoTurnScanTimestamp = blackboard->theFrameInfo.time + 1;
  ...
  for(size_t i = 0; i < xSteps * ySteps; i++)
  {
    ...
    if(!cell->shadowed)
    {
      const unsigned char coverage = cell->coverage(blackboard->theFrameInfo.time);
      if(coverage < worstCoverage)
      {
        worstCoverage = coverage;
        worstIdx = i;
      }
      if(abs(cell->relativeTargetOnField.angle()) < visibleAngle)
      {
  ...
  // Update targets.
  if(lastWorstIdx == -1 || cells[worstIdx]->coverage(blackboard->theFrameInfo.time) < cells[lastWorstIdx]->coverage(blackboard->theFrameInfo.time))
  {
  ...
  else
  {
    fieldCoverage.worstTarget = FieldCoverage::Target(cells[lastWorstIdx]->relativeTargetOnField,
                                cells[lastWorstIdx]->coverage(blackboard->theFrameInfo.time));
  }

  if(lastWorstNoTurnIdx == -1 || abs(cells[lastWorstNoTurnIdx]->relativeTargetOnField.angle()) > visibleAngle + 0.1
     || worstNoTurnScanTimestamp < cells[lastWorstNoTurnIdx]->lastseenScanPattern)
  {
    fieldCoverage.worstNoTurnTarget = FieldCoverage::Target(cells[worstNoTurnIdx]->relativeTargetOnField,
                                      cells[worstNoTurnIdx]->coverage(blackboard->theFrameInfo.time, worstNoTurnScanTimestamp));
  ...
  else
  {
    fieldCoverage.worstNoTurnTarget = FieldCoverage::Target(cells[lastWorstNoTurnIdx]->relativeTargetOnField,
                                      cells[lastWorstNoTurnIdx]->coverage(blackboard->theFrameInfo.time));
  }

  if(lastWorstNoTurnRangeIdx == -1 || worstNoTurnRangeCoverage < cells[lastWorstNoTurnRangeIdx]->coverage(blackboard->theFrameInfo.time)
     || abs(cells[lastWorstNoTurnRangeIdx]->relativeTargetOnField.angle()) > visibleAngle + 0.1
  ...
  else
  {
    fieldCoverage.worstNoTurnRangeTarget = FieldCoverage::Target(cells[lastWorstNoTurnRangeIdx]->relativeTargetOnField,
                                           cells[lastWorstNoTurnRangeIdx]->coverage(blackboard->theFrameInfo.time));
  }

  if(lastWorstNoTurnHalfRangeIdx == -1 || worstNoTurnHalfRangeCoverage < cells[lastWorstNoTurnHalfRangeIdx]->coverage(blackboard->theFrameInfo.time)
     || abs(cells[lastWorstNoTurnHalfRangeIdx]->relativeTargetOnField.angle()) > visibleAngle + 0.1
  ...
  else
  {
    fieldCoverage.worstNoTurnHalfRangeTarget = FieldCoverage::Target(cells[lastWorstNoTurnHalfRangeIdx]->relativeTargetOnField,
        cells[lastWorstNoTurnHalfRangeIdx]->coverage(blackboard->theFrameInfo.time));
  }
  ...
  COMPLEX_DRAWING("module:FieldCoverageProvider:worstNoTurnTargetImage",
  {
    ...
    {
      Vector2<int> img;
      const Vector2<int> abs(static_cast<int>(fieldCoverage.worstNoTurnTarget.target.x), static_cast<int>(fieldCoverage.worstNoTurnTarget.target.y));
      Geometry::calculatePointInImage(abs, theCameraMatrix, theCameraInfo, img);
  ...
  // Assemble Team Comm
  if(blackboard->theTeamMateData.sendThisFrame)
  {
    ...
    for(int i = 0; i < teamData.intervalSize; ++i)
      teamData.cells[i] = cells[intervalOffset + i]->coverage(blackboard->theFrameInfo.time);
    teamData.timestamp = blackboard->theFrameInfo.time;
...
Vector2<> FieldCoverageProvider::projectOnField(int x, int y)
{
  ...
  Geometry::calculatePointOnField(x, y, blackboard->theCameraMatrix, blackboard->theCameraInfo, pointOnField);
  pointOnField = Geometry::relative2FieldCoord(blackboard->theRobotPose, pointOnField);
  return pointOnField;
}
...
void FieldCoverageProvider::calculateMean(FieldCoverage& fieldCoverage)
{
  ...
  for(int i = 0; i < size; ++i)
    sum += fieldCoverage.coverage(i, blackboard->theFrameInfo.time);
...
void FieldCoverageProvider::calculateStdDev(FieldCoverage& fieldCoverage)
{
  ...
  for(int i = 0; i < size; ++i)
    fieldCoverage.stddev += sqr(fieldCoverage.mean - static_cast<float>(fieldCoverage.coverage(i, blackboard->theFrameInfo.time)));
  fieldCoverage.stddev /= static_cast<float>(size);
  fieldCoverage.stddev = sqrt(fieldCoverage.stddev);
}

bool FieldCoverageProvider::isBallThrownIn()
{
  if(blackboard->theGameInfo.timeSinceLastPackageReceived > 1000
     || blackboard->theGameInfo.state != STATE_PLAYING)
  {
    lastGameState = blackboard->theGameInfo.state;
    return false;
  }

  int gameInfoLastThrowIn = blackboard->theFrameInfo.time / 1000 - blackboard->theGameInfo.dropInTime;
     ...
     || gameInfoLastThrowIn > lastThrowInTimestamp + 2)
  {
    ...
    return blackboard->theGameInfo.dropInTime < 30;
  }
  return false;
}
...
void FieldCoverageProvider::ballThrowIn(FieldCoverage& fieldCoverage)
{
  ...
  {
    const bool ownTeamKicked = blackboard->theGameInfo.dropInTeam == blackboard->theOwnTeamInfo.teamColour;
    ...
    if(blackboard->theFrameInfo.getTimeSince(ballOut.time) < 10 * 1000) // We should know where the ball went out.
    {
      if(ballOut.position.x > 2990.f && ballOut.position.x < 3010.f)
        throwInPosition.x = ownTeamKicked ? std::min(throwInPosition.x, 0.f) : xPosOpponentThrowInPoint;
      else if(ballOut.position.x > -3010.f && ballOut.position.x < -2090.f)
        throwInPosition.x = ownTeamKicked ? xPosOwnThrowInPoint : std::max(throwInPosition.x, 0.f);
      else if((ballOut.position.y > 1990.f && ballOut.position.y < 2010.f)
              || (ballOut.position.y > -2010.f && ballOut.position.y < -1090.f))
    ...
    if(throwInPosition.x > blackboard->theFieldDimensions.xPosOpponentGroundline - throwInLineDistanceGL)
      throwInPosition.x = blackboard->theFieldDimensions.xPosOpponentGroundline - throwInLineDistanceGL;
    if(throwInPosition.x < blackboard->theFieldDimensions.xPosOwnGroundline + throwInLineDistanceGL)
      throwInPosition.x = blackboard->theFieldDimensions.xPosOwnGroundline + throwInLineDistanceGL;
    ...
    Vector2<int> throwInCell(static_cast<int>((throwInPosition.x - static_cast<float>(blackboard->theFieldDimensions.xPosOwnGroundline)) / cellLength.x),
                             static_cast<int>((throwInPosition.y - static_cast<float>(blackboard->theFieldDimensions.yPosRightSideline)) / cellLength.y));
    ...
    for(int x = 0; x < static_cast<int>(xSteps); ++x)
    {
      for(int y = 0; y < static_cast<int>(ySteps); ++y)
      {
        ...
        fieldCoverageGrid[x][y]->setCoverage(blackboard->theFrameInfo.time, coverage);
...
{
  for(RobotsModel::RCIt r = blackboard->theRobotsModel.robots.begin(); r != blackboard->theRobotsModel.robots.end(); ++r)
    if(r->standing)
...
bool FieldCoverageProvider::calculateBallOutPosition()
{
  ...
  const float opponentGl = static_cast<float>(blackboard->theFieldDimensions.xPosOpponentGroundline) + 10.f;
  const float ownGl = static_cast<float>(blackboard->theFieldDimensions.xPosOwnGroundline) - 10.f;
  const float leftSl = static_cast<float>(blackboard->theFieldDimensions.yPosLeftSideline) + 10.f;
  const float rightSl = static_cast<float>(blackboard->theFieldDimensions.yPosRightSideline) - 10.f;
    ...
    const CameraInfo& cameraInfo, const ImageCoordinateSystem& imageCoordinateSystem) const
{
  ...
  Geometry::calculatePointInImage(Vector2<int>((int) relPosOnField.x, (int) relPosOnField.y),
                                  cameraMatrix, cameraInfo, pointInImage);
