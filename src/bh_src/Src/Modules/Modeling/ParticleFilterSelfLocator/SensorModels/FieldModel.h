    ...
    void draw()
    {
      for(int y = 0; y < ySize; y += 5)
        for(int x = 0; x < xSize; x += 5)
        {
          ...
          //LINE("module:SelfLocator:fieldModel", x1, y1, closest.x, closest.y, 0, Drawings::ps_solid, ColorClasses::black);
        }
    }
...
public:
  ...
  */
  const Vector2<int> getClosestCorner(const Vector2<int>& point, LinePercept::Intersection::IntersectionType type, float dir) const;
