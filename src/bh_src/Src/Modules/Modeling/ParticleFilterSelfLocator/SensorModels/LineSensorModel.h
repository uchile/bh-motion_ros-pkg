...
{
private:
  const vector<const LinePercept::Line*>& lines; /**< Reference to the lines. */
  vector<short> selectedIndices; /**< The indices of the points in the points percept that were selected for the sensor update. */
  vector< pair<short, float> > selectedIndicesPositions; /**< Precomputed information (distance+angle) about selected points */
...
public:
  ...
  LineSensorModel(const SelfLocatorParameter& selfLocatorParameter,
                  const vector<const LinePercept::Line*>& lines, const FrameInfo& frameInfo,
  ...
  */
                                      ...
                                      const vector<int>& selectedIndices, vector<float>& weightings)
  {
    ...
    for(int j = 0; j < (int) selectedIndices.size(); ++j)
    {
      ...
      const float angle = pp.angle();
      const short distance = static_cast<short>(pp.abs());
      selectedIndicesPositions[j] = make_pair(distance, angle);
    }
      ...
      for(int j = 0; j < (int) selectedIndices.size(); ++j)
      {
        ...
        const Vector2<int>& pp = selectedIndices[j] & 1 ? lines[selectedIndices[j] >> 1]->first
                                 ...
                                 & other = selectedIndices[j] & 1 ? lines[selectedIndices[j] >> 1]->last
                                           ...
                                           pModel = fieldModel.getClosestLinePoint(pObs, pOther, (pp - other).sqr()),
