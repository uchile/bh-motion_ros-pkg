...
private:
  ...
  public:
    ...
    float bearing; /**< Bearing to corner in radians. */
    float direction; /**< Relative direction of corner in radians. */

    SelectedIndex() {}
    SelectedIndex(short distance, float bearing, float direction) :
      distance(distance), bearing(bearing), direction(direction) {}
  };

  const LinePercept& theLinePercept; /**< Reference to points percept */
  vector<short> selectedIndices;    /**< The indices of the points in the points percept that were selected for the sensor update. */
  vector<SelectedIndex> selectedIndicesPositions; /**< Precomputed information (distance+angle) about selected points */
...
public:
  ...
  */
                                      ...
                                      const vector<int>& selectedIndices, vector<float>& weightings)
  {
    ...
    int rotCos  = int(floor(cos(origin.rotation) * (1 << 10) + 0.5f));
    int rotSin  = int(floor(sin(origin.rotation) * (1 << 10) + 0.5f));
    ...
    for(int j = 0; j < (int) selectedIndices.size(); ++j)
    {
      ...
      const float bearing = theLinePercept.intersections[selectedIndices[j]].pos.angle();
      const float direction = theLinePercept.intersections[selectedIndices[j]].type == LinePercept::Intersection::X
                              ? 0
...
NEXT_SAMPLE:
      ...
      for(int j = 0; j < (int) selectedIndices.size(); ++j)
      {
        ...
        weightings[i] *= gaussian.value(
                           abs(1024 * diffFromCamera.x / (abs(ppFromCamera.x) + 450)),
                           theSelfLocatorParameter.standardDeviationCorners);
        weightings[i] *= gaussian.value(
                           ppFromCamera.x != 0 ? abs(1024 * diffFromCamera.y / ppFromCamera.x) : 0,
                           theSelfLocatorParameter.standardDeviationCorners);
      }
    }
FINISHED:
