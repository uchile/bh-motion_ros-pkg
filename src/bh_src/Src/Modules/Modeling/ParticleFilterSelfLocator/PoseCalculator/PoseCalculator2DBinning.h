...
/*
  void draw()
  {
    COMPLEX_DRAWING("module:SelfLocator:poseCalculator",
    {
      const int fieldLength = int(theFieldDimensions.x.getSize());
      const int fieldWidth = int(theFieldDimensions.y.getSize());
      const int squareLength = fieldLength / numberOfGridCells;
      const int squareWidth = fieldWidth / numberOfGridCells;
      // Draw all squares:
...
private:
  void calcPoseFromSubSquare(int xMax, int yMax, RobotPose& robotPose)
  {
    ...
    {
      const float avgX = static_cast<float>(xSum) / sampleCount;
      const float avgY = static_cast<float>(ySum) / sampleCount;
      const float avgRot = atan2(static_cast<float>(sinSum), static_cast<float>(cosSum));
      robotPose = Pose2D(avgRot, avgX, avgY);
      robotPose.validity = (static_cast<float>(sampleCount) / this->samples.size());
    }
  }
};
