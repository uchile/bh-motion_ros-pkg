                             ...
                             const RobotPose& theRobotPose)
{
  ...
  motionUpdate(theOdometryData);
  sensorUpdate(theRobotPercept, theImageCoordinateSystem, theCameraMatrix, theCameraInfo, theFrameInfo, theImage);
  markRobotsToDelete(theFieldDimensions, theRobotPose, theCameraMatrix, theCameraInfo, theImage, theImageCoordinateSystem);
  uniteNearRobots();
  cycle(robotsModel);

  //TEAM_OUTPUT_FAST(idTeamMateRobotsModel, bin, RobotsModelCompressed(robotsModel));
}

void RobotLocatorUKF::motionUpdate(const OdometryData& theOdometryData)
{
...
void RobotLocatorUKF::sensorUpdate(const RobotPercept& theRobotPercept,
                                   const ImageCoordinateSystem& theImageCoordinateSystem,
                                   const CameraMatrix& theCameraMatrix,
                                   const CameraInfo& theCameraInfo,
                                   const FrameInfo& theFrameInfo,
                                   const Image& theImage)
{
  for(RobotPercept::RCIt r(theRobotPercept.robots.begin()); r != theRobotPercept.robots.end(); r++)
    updateMostLikelyOldRobot(*r, theImageCoordinateSystem, theCameraMatrix, theCameraInfo, theFrameInfo);
  for(RobotPercept::RCIt r(theRobotPercept.unlocalizableRobots.begin()); r != theRobotPercept.unlocalizableRobots.end(); r++)
    updateRobotInDirection(*r, theImageCoordinateSystem, theCameraMatrix, theCameraInfo, theImage, theFrameInfo);
}
...
void RobotLocatorUKF::markRobotsToDelete(const FieldDimensions& theFieldDimensions,
                                         const RobotPose& theRobotPose,
                                         const CameraMatrix& theCameraMatrix,
                                         const CameraInfo& theCameraInfo,
                                         const Image& theImage,
