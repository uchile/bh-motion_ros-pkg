...
GroundTruthEvaluator::GroundTruthEvaluator(UChBlackBoard *blackboard)
    :bb(blackboard)
{
}
...
void GroundTruthEvaluator::update(GroundTruthResult& groundTruthResult)
{
  ...
  robotPoses.add(RepresentationWithTimestamp<RobotPose>(bb->theRobotPose,
                 bb->theFrameInfo.time));
  if(groundTruthRobotPoses.getNumberOfEntries() == 0 ||
     groundTruthRobotPoses.top().timestamp != bb->theGroundTruthRobotPose.timestamp)
  {
    groundTruthRobotPoses.add(RepresentationWithTimestamp<RobotPose>(
                                bb->theGroundTruthRobotPose, bb->theGroundTruthRobotPose.timestamp));
  }
  ...
  ballModels.add(RepresentationWithTimestamp<BallModel>(bb->theBallModel,
                 bb->theFrameInfo.time));
  if(groundTruthBallModels.getNumberOfEntries() == 0 ||
     groundTruthBallModels.top().timestamp != bb->theGroundTruthBallModel.timeWhenLastSeen)
  {
    groundTruthBallModels.add(RepresentationWithTimestamp<BallModel>(
                                bb->theGroundTruthBallModel, bb->theGroundTruthBallModel.timeWhenLastSeen));
  }
  ...
  {
    evaluationDelay = (int) bb->theFrameInfo.time - (int) groundTruthRobotPoses[1].timestamp;
  }
...
BallModel GroundTruthEvaluator::averageEstimatedBallModel()
{
  ...
  if(groundTruthBallModels.getNumberOfEntries() >= 3)
  {
    ...
    for(size_t i = 0; i < ballModels.getNumberOfEntries(); i++)
    {
      ...
      averagedEstimatedBallModel.lastPerception.position += ballModels[i].lastPerception.position;
      aggregatedModels++;
    }
    ...
    if(aggregatedModels)
    {
      ...
      averagedEstimatedBallModel.lastPerception.position /= (float) aggregatedModels;
    }
  }
