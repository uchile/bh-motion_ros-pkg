...
private:
  ...
  */
  ...
  BallState getCombinedBallPosition(bool& ballIsValid
                                    , const RobotPose& theRobotPose
                                    , const RobotInfo& theRobotInfo
                                    , const FieldDimensions& theFieldDimensions
                                    , const TeamMateData& theTeamMateData
                                    , const BallHypotheses& theBallHypotheses
                                    , const FrameInfo& theFrameInfo
                                    , const BallModel& theBallModel); // calculates the global ball position
  float computeWeights(const ExtendedBallModel& ballModel, const RobotPose& robotPose, const BallHypotheses& ballHypotheses, const FrameInfo& theFrameInfo) const; // computes weight for the global ball position
  void clusterAllDetectedRobots(); // clusters all detected robots
  vector<GaussianDistribution> getPositionOfOpponentRobots(const FieldDimensions& theFieldDimensions); // calculates the positions and covariance of the opponent robots by using the clusters. All positions inside one cluster are merged by using the last Kalmafilter step.
