...
public:
  ...
  */
  void update(USObstacleGrid& usObstacleGrid, const OdometryData& theOdometryData, const GameInfo& theGameInfo,
...
private:
  ...
  void checkUS(const FilteredSensorData& theFilteredSensorData, const FrameInfo& theFrameInfo, const OdometryData& theOdometryData);
  ...
  */
  void addUsMeasurement(bool actuatorChanged, float m, SensorData::UsActuatorMode mappedActuatorMode, unsigned int timeStamp,
  ...
  void ageCellState(const FrameInfo& theFrameInfo);

  /** Called by occupyCells, frees the space between the sensor and the obstacle */
  void fillScanBoundary(const FrameInfo& theFrameInfo);
  ...
  */
  Vector2<int> worldToGrid(const Vector2<int>& p, const OdometryData& theOdometryData)const;
  ...
  */
  Vector2<int> gridToWorld(const Vector2<int>& p, const OdometryData& theOdometryData) const;

  /** Converts coordinates in grid coordinates to the local coordinate system
  *   Floating point version, allows to convert coordinates with sub-cell precision
  * @param p The position within the grid
  * @return A position in local robot coordinates
  */
  ...
  */
  void line(const Vector2<int>& start, const Vector2<int>& end, const FrameInfo& theFrameInfo);

  /** Moves the grid cells according to the odometry differences*/
  void moveGrid(const OdometryData& theOdometryData);
