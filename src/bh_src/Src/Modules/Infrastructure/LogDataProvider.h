...
#define HANDLE2(representation, cmd) \
  ...
  { \
    ...
    if (getLogConverter()->isConvertRequired(message, id##representation)) { \
      if (representationBuffer[id##representation]) \
        delete representationBuffer[id##representation]; \
      representationBuffer[id##representation] = getLogConverter()->newConvertedRepresentation(message, id##representation); \
    } \
    else \
