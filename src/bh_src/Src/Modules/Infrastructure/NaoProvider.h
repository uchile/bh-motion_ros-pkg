...
//  NaoBody naoBody;
  ...
  void update(JointData& jointData, const JointCalibration &theJointCalibration, const SensorCalibration &theSensorCalibration);
  void update(SensorData& sensorData) {sensorData = this->sensorData;}
  void update(KeyStates& keyStates) {keyStates = this->keyStates;}
  void update(FrameInfo& frameInfo,const JointData& theJointData) {frameInfo.time = theJointData.timeStamp; frameInfo.cycleTime = theJointData.cycleTime;}
...
public:
  ...
  */
  ...
  //float actuators[2*JointData::numOfJoints];numOfActuatorIds
  float actuators[120]; //Colocar el numero de actuadores que corresponda
  // position, current, temperature for each joint and sensor information
  float sensors[3*JointData::numOfJoints + SensorData::numOfSensors + 62 ];
  /**
