...
#define INIT_DEBUG_IMAGE(id, image) \
  NOT_POLLABLE_DEBUG_RESPONSE("debug images:" #id , id##Image = image; )
...
#define SEND_DEBUG_GRAY_SCALE_IMAGE(id) \
  DEBUG_RESPONSE("debug images:" #id , OUTPUT(idDebugGrayScaleImage, bin, #id << id##Image); )

// color class

/**Sends the color class debug image with the specified id*/
#define SEND_DEBUG_COLOR_CLASS_IMAGE(id) \
  DEBUG_RESPONSE("debug images:" #id , OUTPUT(idDebugColorClassImage, bin, #id << id##Image); )
