...
BresenhamLineScan::BresenhamLineScan(const Vector2<>& direction)
{
  setup(Vector2<int>(static_cast<int>(direction.x * 1024.0),
                     static_cast<int>(direction.y * 1024.0)));
}

BresenhamLineScan::BresenhamLineScan(const float& direction)
{
  setup(Vector2<int>(static_cast<int>(cos(direction) * 1024.0),
                     static_cast<int>(sin(direction) * 1024.0)));
}

BresenhamLineScan::BresenhamLineScan(const Vector2<int>& start, const Vector2<>& direction, const CameraInfo& cameraInfo)
{
  // Create a line through the point start with the given direction
  Geometry::Line line(Vector2<>(static_cast<float>(start.x), static_cast<float>(start.y)),
                      direction);
  ...
  Vector2<> delta(float(intersection1.x - start.x), float(intersection1.y - start.y));
  ...
  else
  {
    ...
    delta.x = float(intersection2.x - start.x);
    delta.y = float(intersection2.y - start.y);
    if(delta * direction > 0)
      setup(intersection2 - start);
    else
      setup(intersection1 - start);
...
BresenhamLineScan::BresenhamLineScan(const Vector2<int>& start, const float& direction, const CameraInfo& cameraInfo)
{
  ...
  Geometry::Line scanLine(start, Vector2<>(cos(direction), sin(direction)));
  Vector2<int> lineStart, lineEnd;
  Geometry::getIntersectionPointsOfLineAndRectangle(frameUpperLeft,
                                                    frameLowerRight,
                                                    scanLine, lineStart, lineEnd);
  Vector2<> delta((float)(lineEnd - start).x, (float)(lineEnd - start).y);
  Vector2<> directionVector(cos(direction), sin(direction));
