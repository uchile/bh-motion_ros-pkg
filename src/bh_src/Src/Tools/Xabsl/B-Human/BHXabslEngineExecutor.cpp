/**
* @file BHXabslEngineExecutor.cpp
*
...
#include "BHXabslEngineExecutor.h"
//#include "Tools/Debugging/Debugging.h"
#include "Platform/BHAssert.h"
#include "Platform/SystemCall.h"
#include <iostream>

//PROCESS_WIDE_STORAGE(BHXabslEngineExecutor) BHXabslEngineExecutor::theInstance = 0;


BHXabslEngineExecutor::BHXabslEngineExecutor(const char* id, const unsigned int& time)
  : pEngine(0), errorHandler(id), id(id), time(time)
{
  //theInstance = this;
}

BHXabslEngineExecutor::~BHXabslEngineExecutor()
{
  if(pEngine != 0) delete pEngine;
}

void BHXabslEngineExecutor::init()
{
  //char filename[100];
  //sprintf(filename, "Xabsl/%s-ic.dat", id);
  XabslFileInputSource input("/home/nao/.config/naoqi/Data/Config/Xabsl/bh2011-ic.dat");
  init(input);
}

void BHXabslEngineExecutor::init(xabsl::InputSource& input)
{
  if(pEngine != 0) delete pEngine;
  ...
  std::cout << "time: " << getCurrentSystemTime() << std::endl;


#ifndef WIN32
  pEngine = new xabsl::Engine(errorHandler,&SystemCall::getCurrentSystemTime/*(unsigned int (*)(void))&time(&BHXabslEngineExecutor::getCurrentSystemTime)*/);
#else
  pEngine = new xabsl::Engine(errorHandler, (unsigned int (__cdecl*)(void))(&BHXabslEngineExecutor::getCurrentSystemTime));
#endif
...
void BHXabslEngineExecutor::executeEngine()
{
  if(errorHandler.errorsOccurred)
  {
  ...
  //DEBUG_RESPONSE("automated requests:xabsl:debugMessages", sendDebugMessage(Global::getDebugOut()););
  //DEBUG_RESPONSE("automated requests:xabsl:debugSymbols", sendDebugSymbols(Global::getDebugOut()););
}

void BHXabslEngineExecutor::sendActiveOptionsToStream(Out& out) const
{
  int i, j, k;
...
/*void BHXabslEngineExecutor::sendDebugMessage(OutMessage& outMessage) const
{
  int i, j;
  Out& out = outMessage.bin;
  ...
  outMessage.finishMessage(idXabslDebugMessage);
}

void BHXabslEngineExecutor::sendDebugSymbols(OutMessage& outMessage) const
{
  int i, j;
  Out& out = outMessage.bin;
  // send the id of the behavior contol solution
  ...
  outMessage.finishMessage(idXabslDebugSymbols);
}

bool BHXabslEngineExecutor::handleXabslMessage(InMessage& message)
{
    ...
    for(i = 0; i < numberOfChangedOutputSymbols; i++)
    {
      ...
      case 'e':
        ...
        else
        {
          ...
          {
            //OUTPUT(idText, text, "Enum element " << buf2 << " for symbol " << buf<< " does not exist");
          }
          else
          {
...
bool BHXabslEngineExecutor::setSelectedAgent(const char* name)
{
  if(pEngine != 0)
...
BHXabslErrorHandler::BHXabslErrorHandler(const char* id)
  : id(id)
{
}

void BHXabslErrorHandler::printError(const char* text)
{
  /*OUTPUT(idText, text, "("
         << id
...
void BHXabslErrorHandler::printMessage(const char* text)
{
  /*OUTPUT(idText, text, "("
         << id
         << " xabsl::Engine): " << text);*/
...
float XabslFileInputSource::readValue()
{
  float d = 0;
  if(file != 0) *file >> d;
  return d;
...
float XabslMessageInputSource::readValue()
{
  float d;
  message >> d;
  return d;
