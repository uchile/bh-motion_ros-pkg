...
void Parameters::registerDecimal(const char* name, float& parameter)
{
...
bool ParameterAssignment::setDecimalParameter(const char* param, float value)
{
...
bool ParameterAssignment::set()
{
  ...
  {
    float newValue = decimalExpressions[i]->getValue();
    decimalValues.setElement(i, newValue);
    if(memcmp(decimal[i], &newValue, sizeof(float)))
    {
